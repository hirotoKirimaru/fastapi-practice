...
class TestUser:
    ...
    class TestName:
        async def test_01(self, db: AsyncSession):
            user1 = User(id=1, name="kirimaru", email="a@example.com", organization_id = 1)
            db.add(user1)
            await db.commit()
        ...
        async def test_soft_destroyed_name(self, db: AsyncSession):
            user1 = User(id=1, name="kirimaru", email="a@example.com", organization_id = 1, soft_destroyed_at=datetime.now())
            db.add(user1)
            await db.commit()
